...
proc create-cbr-connection { src dst } {
	...
	puts "\$ns attach-agent \$n($src) \$udp_($cbr_cnt)"
	puts "set null_($cbr_cnt) \[new Agent/Null\]"
	puts "\$ns attach-agent \$n($dst) \$null_($cbr_cnt)"
	...
	puts "\$ns connect \$udp_($cbr_cnt) \$null_($cbr_cnt)"
	
	puts "\$ns at $stime \"\$cbr_($cbr_cnt) start\""
